...
UPDATE HOADON
SET TONGTIEN = DBO.uf_LT_CNTONGTIEN_HOADON(MAPDP)
go
...
DROP PROCEDURE IF EXISTS UPDATE_TTPHONG_TTHOADON
go
CREATE PROCEDURE UPDATE_TTPHONG_TTHOADON
    @MAHD INT,
    @TINHTRANG NVARCHAR(16) = N'Đã thanh toán',
    @TINHTRANGPHONG NVARCHAR(15) = N'Chưa dọn'
AS
BEGIN
    -- Kiểm tra tham số @MAHD không null
    IF @MAHD IS NULL
    BEGIN
        PRINT 'Giá trị của tham số @MAHD không được để trống.'
        RETURN
    END

    -- Cập nhật cột TINHTRANGTT trong bảng HOADON
    UPDATE HOADON
    SET TINHTRANGTT = @TINHTRANG, TONGTIEN = dbo.uf_LT_CNTONGTIEN_HOADON(@MAHD)
    WHERE MAHD = @MAHD

    -- Cập nhật cột TINHTRANGPHONG trong bảng PHONG
    UPDATE PHONG
    SET TINHTRANG = @TINHTRANGPHONG
    FROM PHONG
    INNER JOIN CHITIETPHIEUDATPHONG ON PHONG.MAPHONG = CHITIETPHIEUDATPHONG.MAPHONG
    INNER JOIN PHIEUDATPHONG ON CHITIETPHIEUDATPHONG.MAPDP = PHIEUDATPHONG.MAPDP
    INNER JOIN HOADON ON PHIEUDATPHONG.MAPDP = HOADON.MAPDP
    WHERE HOADON.MAHD = @MAHD
END
GO
---
--EXEC UPDATE_TTPHONG_TTHOADON @MAHD = 3
